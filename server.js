...
async function randomPano(exclude=''){let seeds=[...PANO_SEEDS].sort(()=>Math.random()-.5);for(const seed of seeds.slice(0,7)){try{let u=`https://api.openstreetcam.org/2.0/photo/?lat=${seed.lat}&lng=${seed.lng}&zoomLevel=13`;let c=new AbortController(),t=setTimeout(()=>c.abort(),6500),r=await fetch(u,{headers:{'User-Agent':'LandmarkDuel/21.0'},signal:c.signal});clearTimeout(t);if(!r.ok)continue;let j=await r.json(),arr=deepPhotos(j).filter(x=>String(x.id||x.photoId||x.fileurlProc||x.fileurl)!==exclude);if(!arr.length)continue;let x=arr[Math.floor(Math.random()*arr.length)],url=x.fileurlProc||x.fileurl||x.fileUrlProc||x.fileUrl;if(!/^https:\/\//.test(url))continue;return {id:String(x.id||x.photoId||url),lat:+x.lat,lng:+x.lng,heading:+(x.heading||x.compassAngle||0),country:seed.country,flag:seed.flag,url}}catch{}}return {id:'fallback-jakarta',lat:-6.193911,lng:106.849350,heading:150.16,country:'Indonesia',flag:'🇮🇩',url:'https://storage13.openstreetcam.org/files/photo/2022/10/6/wrapped_proc/6187609_626b8_633f5d392b4be.jpg'}}
...
const server=http.createServer(async(req,res)=>{const u=new URL(req.url,'http://localhost');
 ...
 if(u.pathname==='/health'||u.pathname==='/api/health'){res.writeHead(200,{'Content-Type':'application/json','Cache-Control':'no-store'});return res.end(JSON.stringify({ok:true,mode:'geo-duel-v21.0-stable-p2p-twitch',rooms:'browser-p2p'}));}
...
server.listen(PORT,()=>console.log('Landmark Duel V21.0 STABLE on '+PORT));
