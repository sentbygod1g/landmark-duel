...
const server=http.createServer(async(req,res)=>{const u=new URL(req.url,'http://localhost');
 ...
 if(u.pathname==='/health'||u.pathname==='/api/health'){res.writeHead(200,{'Content-Type':'application/json','Cache-Control':'no-store'});return res.end(JSON.stringify({ok:true,mode:'geo-duel-v21.4-best-of-5-flag-hud',rooms:'browser-p2p'}));}
...
server.listen(PORT,()=>console.log('Landmark Duel V21.4 on '+PORT));
